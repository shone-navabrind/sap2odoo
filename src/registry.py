...
REGISTRY = [
    ...
    # --- Sales ---
    ObjectSpec(57, "Sales", "Master", "Products", True, "product.template", "product_template", "built", "REAL DATA, FULL FIELD COVERAGE: all real per-material entities in vmumaterial checked (78 entity sets total) - MaterialCollection (base), TextCollection (detailed description, 2172/3058), PurchasingCollection/SalesCollection (UOM + purchase_ok/sale_ok), ProductCategoryCollection (category, 3058/3058 resolved), vmumaterialvaluationdata (cost price, 994/3058). No barcode/weight/dimension data exists on this tenant (GlobalTradeItemNumberCollection and QuantityCharacteristicCollection both confirmed 0 rows live) - not fabricated, left blank.",
               raw_sources=("vmumaterial__MaterialCollection.json", "vmumaterial__TextCollection.json", "vmumaterial__PurchasingCollection.json", "vmumaterial__SalesCollection.json", "vmumaterial__ProductCategoryCollection.json", "vmumaterialvaluationdata__MaterialValuationDataCollection.json", "vmumaterialvaluationdata__ValuationPriceCollection.json")),
