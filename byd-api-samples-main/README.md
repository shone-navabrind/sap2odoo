...
> **Before running any extraction script in this project:** the Custom OData Services in this
> folder must be imported and activated in SAP first, or the extractor has nothing to pull from.
>
> In SAP Business ByDesign: **Application and User Management** work center → **OData Services**
> view → **Custom OData Services** → upload/import each `.xml` file from the
> `Custom OData Services` folder below → activate it. Only services that are imported this way
> become live, callable OData endpoints (`/sap/byd/odata/cust/v1/<service_name>/...`) - an
> un-imported `.xml` file sitting in this folder does nothing on its own.
>
> Import as many as you need for the sheet objects still `pending_mapping` in the parent
> project's `src/registry.py` / `PROJECT_STATUS.csv` - not every file here maps to something on
> the migration sheet, see that file for what's actually needed.
...
1. Install [Postman API Development Environment](https://www.getpostman.com/) (only needed if you want to explore the Postman collections directly - the parent project's Python scripts don't need Postman)
...
This is SAP's own sample content, provided "as-is"; no support is available from SAP for it.
Original source: [SAP-samples/byd-api-samples](https://github.com/SAP-samples/byd-api-samples)
(Apache 2.0 licensed there - license/REUSE metadata files were removed here since this folder
now lives inside a different project's repo; refer to the original repo for full license text).
